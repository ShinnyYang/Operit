...
const BaiduMapAssistant = (function () {
    ...
    async function ensureMain(packageName = BAIDU_MAP_PACKAGE) {
    ...
    async function search_location(params) {
    ...
    async function get_map_search_results(desiredCount = 20) {
        const results = [];
        ...
        for (let i = 0; i < maxScrolls; i++) {
            ...
            for (const button of goToButtons) {
                ...
                const item = button.closest((node) => node.parent !== undefined && node.parent.equals(recyclerView));
                if (!item)
                    continue;
                ...
                // 查找包含地址特征的文本作为地址
                const addressNode = relevantTextViews.find(tv => tv.text && (tv.text.includes('省') || tv.text.includes('市') || tv.text.includes('区') || tv.text.includes('路') || tv.text.includes('号')));
    ...
    async function select_location_from_list(params) {
    ...
    async function go_to_location(params) {
    ...
    async function get_directions(params) {
    ...
    async function activate_voice_assistant(params) {
