...
import {
  ensureDeepSeekHarnessWebServer,
  type DeepSeekHarnessWebServerProgressEvent,
...
export default function Screen(ctx: ComposeDslContext): ComposeNode {
  ...
  async function startRuntime(forceRestart: boolean): Promise<void> {
    setLoading(true);
    setErrorText("");
    setProgress(5);
    try {
      const result: DeepSeekHarnessWebServerResult = await ensureDeepSeekHarnessWebServer({
        forceRestart,
        onProgress: (event: DeepSeekHarnessWebServerProgressEvent) => {
